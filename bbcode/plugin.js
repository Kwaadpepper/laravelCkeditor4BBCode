...
(function() {
  ...
  const blockLikeTags = CKEDITOR.tools.extend( {table: 1}, dtd.$block, dtd.$listItem, dtd.$tableContent, dtd.$list );

  const semicolonFixRegex = /\s*(?:;\s*|$)/;

  /**
   * Serialize Style css prop
   * @param {*} stylesObject
  ...
  function serializeStyleText( stylesObject ) {
    ...
    for (const style in stylesObject) {
      if ({}.hasOwnProperty.call(stylesObject, style)) {
        const styleVal = stylesObject[style];
        const text = ( style + ':' + styleVal ).replace( semicolonFixRegex, ';' );
        styleText += text;
      }
    }
    return styleText;
  ...
  /**
   * unSerialize Style css prop
   * @param {*} styleText
   * @return {String} unserialized
   */
  function usSerializeStyleText( styleText ) {
    const stylesArray = styleText.replace(/;$/, '').split(';');
    const styleObj = {};

    for (const style in stylesArray) {
      if ({}.hasOwnProperty.call(stylesArray, style)) {
        const tmp = stylesArray[style].split(':');
  ...
  const smileyMap = {smiley: ':)', sad: ':(', wink: ';)', laugh: ':D', cheeky: ':P', blush: ':*)', surprise: ':-o', indecision: ':|', angry: '>:(', angel: 'o:)', cool: '8-)', devil: '>:-)', crying: ';(', kiss: ':-*'};
  // jscs:enable maximumLineLength
  const smileyReverseMap = {};
  let smileyRegExp = [];

  // Build regexp for the list of smiley text.
  for ( const i in smileyMap ) {
  ...
  CKEDITOR.BBCodeParser.prototype = {
    parse: function( bbcode ) {
      ...
      while ( ( parts = this._.bbcPartsRegex.exec( bbcode ) ) ) {
        const tagIndex = parts.index;
        if ( tagIndex > lastIndex ) {
          const text = bbcode.substring( lastIndex, tagIndex );
          this.onText( text, 1 );
        }
        ...
        // Opening tag
        if ( parts[1] ) {
          const tagName = bbcodeMap[part];
          const attribs = {};
          const styles = {};
          let optionPart = parts[2];
          ...
          if ( optionPart ) {
            ...
            if ( stylesMap[part] ) {
              ...
              // Handle alt attr and other styles
              const __decodedStyle = usSerializeStyleText(optionPart);
              // Set attributes
              for (const key in imageAttributes) {
                if ({}.hasOwnProperty.call(imageAttributes, key)) {
                  attribs[imageAttributes[key]] = __decodedStyle[imageAttributes[key]] || '';
                  delete __decodedStyle[imageAttributes[key]];
                }
              }
              // Set style values
              attribs['style'] = '';
              for (const attr in __decodedStyle) {
                if ({}.hasOwnProperty.call(__decodedStyle, attr)) {
                  attribs['style'] += `${attr}:${__decodedStyle[attr] || ''};`;
                }
              }
            }
          }
  ...
  /**
	 ...
	 * @return {CKEDITOR.htmlParser.fragment} The fragment created.
	 */
  CKEDITOR.htmlParser.fragment.fromBBCode = function( source ) {
    const parser = new CKEDITOR.BBCodeParser();
    const fragment = new CKEDITOR.htmlParser.fragment();
    let pendingInline = [];
    let pendingBrs = 0;
    ...
    function checkPending( newTagName ) {
      if ( pendingInline.length > 0 ) {
        for ( let i = 0; i < pendingInline.length; i++ ) {
          let pendingElement = pendingInline[i];
          const pendingName = pendingElement.name;
          const pendingDtd = CKEDITOR.dtd[pendingName];
          const currentDtd = currentNode.name && CKEDITOR.dtd[currentNode.name];
    ...
    function checkPendingBrs( tagName, closing ) {
      ...
      let previous = len > 0 && currentNode.children[len - 1];
      const lineBreakParent = !previous && writer.getRule( tagnameMap[currentNode.name], 'breakAfterOpen' );
      const lineBreakPrevious = previous && previous.type == CKEDITOR.NODE_ELEMENT && writer.getRule( tagnameMap[previous.name], 'breakAfterClose' );
      const lineBreakCurrent = tagName && writer.getRule( tagnameMap[tagName], closing ? 'breakBeforeClose' : 'breakBeforeOpen' );
    ...
    function addElement( node, target ) {
      ...
      const previous = len > 0 && target.children[len - 1] || null;
    ...
    parser.onTagOpen = function(tagName, attributes) {
      const element = new CKEDITOR.htmlParser.element(tagName, attributes);
      ...
      const currentDtd = currentName && ( CKEDITOR.dtd[currentName] || ( currentNode._.isBlockLike ? CKEDITOR.dtd.div : CKEDITOR.dtd.span ) );

      // If the element cannot be child of the current element.
      if ( currentDtd && !currentDtd[tagName] ) {
        let reApply = false;
        let addPoint; // New position to start adding nodes.
    ...
    parser.onTagClose = function( tagName ) {
      ...
      const pendingAdd = [];
      const newPendingInline = [];
      let candidate = currentNode;
    ...
    parser.onText = function(text) {
      const currentDtd = CKEDITOR.dtd[currentNode.name];
  ...
  const BBCodeWriter = CKEDITOR.tools.createClass( {
    $: function() {
      this._ = {
        output: [],
        rules: [],
    ...
    proto: {
      ...
      setRules: function( tagName, rules ) {
        const currentRules = this._.rules[tagName];
      ...
      write: function() {
        ...
        const data = Array.prototype.join.call( arguments, '' );
      ...
      getHtml: function( reset ) {
        const bbcode = this._.output.join( '' );
  ...
  CKEDITOR.plugins.add( 'bbcode', {
    ...
    init: function( editor ) {
      ...
      editor.dataProcessor.htmlFilter.addRules( {
        elements: {
          $: function(element) {
            ...
            const style = CKEDITOR.tools.parseCssText( attributes.style, 1 );
            let value;
            ...
            if ( tagName in convertMap ) {
              ...
              if ( ( value = style.color ) ) {
                ...
                const percentValue = value.match( /(xx\-small|x\-small|small|medium|large|x\-large|xx\-large)/ );
                if ( percentValue ) {
              ...
              try {
                ...
                const quoted = element.children[1];
                const citeText = cite.name == 'cite' && cite.children[0].value;
              ...
              if ( ( value = attributes.href ) ) {
                if ( value.indexOf( 'mailto:' ) !== -1 ) {
                  ...
                  const singleton = element.children.length == 1 && element.children[0];
              ...
              const src = attributes['data-cke-saved-src'] || attributes.src;
              ...
              if ( src && src.indexOf( editor.config.smiley_path ) != -1 && alt ) {
                ...
                // is any other image'
                // Set atributes to value first
                value = '';
                for (const key in imageAttributes) {
                  if ({}.hasOwnProperty.call(imageAttributes, key)) {
                    value += `${imageAttributes[key]}:${attributes[imageAttributes[key]]};`;
                  }
                }
                // Now set style values
                for (const attr in style) {
                  if ({}.hasOwnProperty.call(style, attr)) {
                    value += `${attr}:${style[attr] || (attr === 'id' ? '#' : 'inherit')};`;
                  }
                }
          ...
          div: function( element ) {
            const alignment = CKEDITOR.tools.parseCssText( element.attributes.style, 1 )['text-align'] || '';
      ...
      function onSetData( evt ) {
        const bbcode = evt.data.dataValue;
      ...
      if ( editor._.elementsPath ) {
        ...
        if ( ( filters = editor._.elementsPath.filters ) ) {
          filters.push( function( element ) {
            const htmlName = element.getName();
            let name = tagnameMap[htmlName] || false;
            ...
            // Styled span could be either size or color.
              ...
              const src = element.data( 'cke-saved-src' ) || element.getAttribute( 'src' );
  ...
  // Hack to parse smileys on write
  CKEDITOR.buffer = '';
  CKEDITOR.buffer3 = '';
  CKEDITOR.buffer4 = '';
  const replaceByEmoji = function(buffer, editor, config) {
    ...
    element.attr('data-cke-saved-src', src);
    element.attr('title', description);
    element.attr('alt', description);


    /**
           * replace emoji text with html emoji eg:  ;)
           */
    const sel = editor.getSelection();
    if (typeof sel.getRanges()[0] !== 'undefined') {
      const ranges = sel.getRanges();
      const last = ranges[0].startContainer;
      ranges[0].setStart(last, ranges[0].startOffset - buffer.length);
      ranges[0].setEnd(last, ranges[0].endOffset); // range
      sel.selectRanges(ranges);
      editor.insertHtml(element[0].outerHTML);
      // editor.getSelection().removeAllRanges();
    }
  };

  const resetBuffers = function() {
    // console.log('reset buffers');
    // console.log('buffer 2 : "'+CKEDITOR.buffer+'"');
    // console.log('buffer 3 : "'+CKEDITOR.buffer3+'"');
    // console.log('buffer 4 : "'+CKEDITOR.buffer4+'"');
    CKEDITOR.buffer = '';
    CKEDITOR.buffer3 = '';
    CKEDITOR.buffer4 = '';
  };
  ...
  const registerEvent = function() {
    const instance = CKEDITOR.instances[Object.keys(CKEDITOR.instances)[0]];
    const editor = instance.ui.editor;
    const config = editor.config;
    editor.document.on('keyup', function(event) {
      // console.log(event.data.$.key);
      if (event.data.$.key.length === 1) {
        CKEDITOR.buffer += event.data.$.key;
        CKEDITOR.buffer3 += event.data.$.key;
        CKEDITOR.buffer4 += event.data.$.key;
      } else if (event.data.$.key !== 'Shift') {
        resetBuffers();
      }
      if (CKEDITOR.buffer4.length > 4) {
        CKEDITOR.buffer4 = CKEDITOR.buffer4.slice(1);
      }
      if (CKEDITOR.buffer3.length > 3) {
        CKEDITOR.buffer3 = CKEDITOR.buffer3.slice(1);
      }
      if (CKEDITOR.buffer.length > 2) {
        CKEDITOR.buffer = CKEDITOR.buffer.slice(1);
      }
      if (typeof smileyReverseMap[CKEDITOR.buffer4] !== 'undefined') {
        // console.log('buffer 4 match');
        replaceByEmoji(CKEDITOR.buffer4, editor, config);
        resetBuffers();
      }
      if (typeof smileyReverseMap[CKEDITOR.buffer3] !== 'undefined') {
        // console.log('buffer 3 match');
        replaceByEmoji(CKEDITOR.buffer3, editor, config);
        resetBuffers();
      }
      if (typeof smileyReverseMap[CKEDITOR.buffer] !== 'undefined') {
        // console.log('buffer 2 match');
        replaceByEmoji(CKEDITOR.buffer, editor, config);
        resetBuffers();
      }
